...
public class RealtimeNotificationController {
    ...
    @Operation(summary = "SSE 통신 연결 요청", description = "이벤트 내용을 전달하기 위한 신시간통신을 연결합니다.")
    ...
    public ResultResponse<SseEmitter> connect(@RequestHeader(value = "Last-Event-ID", required = false, defaultValue = "") final String lastEventId) {
        ...
        String email = SecurityUtil.getSignInMemberEmail();
        log.info("{}님에게서 신시간 통신 연결 요청이 수신되었습니다.", email);

        SseEmitter emitter = realtimeSender.connect(memberId, lastEventId);

        log.info("{}님에게서 신시간 통신 연결에 처리가 완료되었습니다.", email);
        return ResultResponse.of(ResultCode.REALTIME_CONNECTION_SUCCESS, emitter);
    }
}
